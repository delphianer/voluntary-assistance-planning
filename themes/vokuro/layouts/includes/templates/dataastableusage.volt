 {#  how?
    ...
    {% for collectData in INSERT_COLLECTION_OBJECT_HERE %}
        ...
        {% if collectData.INSERT_DECISION_VAR_HERE == 'INSERT_DECISION_VALUE_HERE' %}
            ...
            {% do arrayPush(rowData , [ 'data' : INSERT_COL_2_DATA_THAT_COMES_FROM_TRUE_DECISION_HERE, 'class' : 'text-center INSERT_EXTRA_CLASSES_IF_NEEDED_HERE'] ) %}

        {% else %}

            {% do arrayPush(rowData , [ 'data' : INSERT_COL_2_DATA_THAT_COMES_FROM_FALSE_DECISION_HERE, 'class' : 'text-center INSERT_EXTRA_CLASSES_IF_NEEDED_HERE'] ) %}
