...
<div class="btn-group mb-5" role="group">
    ...
    {{ link_to(url("operationshifts_departments_link/new"), "Create operationshifts_departments_link", 'class': 'btn btn-primary') }}
</div>
...
<table class="table table-bordered table-striped">
    <thead>
        <tr>
            ...
            <th>NumberVolunteersNeeded</th>
            <th>MinimumCertificateRanking</th>
    ...
    {% else %}
        <tr>
            <td colspan="10">
                No operationshifts_departments_link are recorded
            </td>
        </tr>
    {% endfor %}
    </tbody>
    ...
    <tr>
        <td colspan="10" class="text-right">
            <div class="btn-group" role="group">
                {{ link_to(url("operationshifts_departments_link/search") , '<i class="icon-fast-backward"></i> First', "class": "btn btn-secondary") }}
                {{ link_to(url("operationshifts_departments_link/search?page=") ~ page.previous, '<i class="icon-step-backward"></i> Previous', "class": "btn btn-secondary") }}
                {{ link_to(url("operationshifts_departments_link/search?page=") ~ page.next, '<i class="icon-step-forward"></i> Next', "class": "btn btn-secondary") }}
                {{ link_to(url("operationshifts_departments_link/search?page=") ~ page.last, '<i class="icon-fast-forward"></i> Last', "class": "btn btn-secondary") }}
            </div>
